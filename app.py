...
# رابط السوبابيس مالج شغال وفنان
...
class Region(db.Model):
    ...
    # علاقات عشان نجيب السيارات والسجلات الخاصة بكل منطقة بسهولة
...
class VehicleRecord(db.Model):
    id = db.Column(db.String(50), primary_key=True) # بنستخدم الـ timestamp كـ ID مثل كودج القبلي
...
# السطر السحري اللي بيصنع الجداول فوراً في Supabase أول ما يشتغل الموقع
...
def region_details(name):
    ...
    # ترتيب السجلات عشان الأحدث يظهر فوق أول شي
...
def manage_cars():
    ...
    if region:
        if action == "add" and value:
            # نتأكد إن السيارة مب مضافة للمنطقة هاي من قبل
