...
function printQR() {
    ...
    win.document.write('<html><head><title>Print QR</title><style>body{font-family:Arial;text-align:center;padding:20px}</style></head><body>');
    win.document.write(document.getElementById('printableQR').innerHTML);
...
function toggleMenu(region) {
    document.querySelectorAll('.menu').forEach(m => m.id !== 'menu-'+region && m.classList.remove('show'));
